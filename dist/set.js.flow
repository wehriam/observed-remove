...
class ObservedRemoveSet<T> extends EventEmitter {
  declare map: ObservedRemoveMap<string, T>;

  declare dequeue: () => void;
  declare publish: () => void;
  declare flush: () => void;
  declare sync: (?[Array<[string, number, T | void]>, Array<[number, string]>, number]) => void;
  declare dump: () => [Array<[string, number, T | void]>, Array<[number, string]>, number];
  declare process: ([Array<[string, number, T | void]>, Array<[number, string]>, number]) => void;
  declare clear: () => void;
  ...
  constructor(entries?: Iterable<T>, options?:Options = {}) {
    ...
    this.map.on('set', (key:string, value:T) => { // eslint-disable-line no-unused-vars
      this.emit('add', value);
    });
    this.map.on('affirm', (key:string, value:T) => { // eslint-disable-line no-unused-vars
      this.emit('affirm', value);
    });
    this.map.on('delete', (key:string, value:T) => { // eslint-disable-line no-unused-vars
      this.emit('delete', value);
    });
    this.map.on('publish', (queue:[Array<[string, number, T | void]>, Array<[number, string]>, number]) => { // eslint-disable-line no-unused-vars
