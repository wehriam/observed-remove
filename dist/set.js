...
class ObservedRemoveSet    extends EventEmitter {
   ...
   */
  process(queue           ) {
    const queueWithHashes = rawQueue.map(([id       , value    ]) => {
      if (value && id) {
    ...
    for (const [id       , value    , hash       ] of queueWithHashes) { // eslint-disable-line no-unused-vars
      if (!value) {
        continue;
      }
    ...
    for (const [id       , value    , hash       ] of queueWithHashes) {
      if (!value) {
        continue;
      }
    ...
    for (const [id       , value    ] of queueWithHashes) {
      if (value) {
        continue;
      }
